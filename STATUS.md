...
- **P2-2** (часть 2: async-pipeline включён) После этапов B+C+D `register_devin.main()` теперь идёт через `asyncio.run(main_async(...))`. Новые функции в `devin_async.py`: `start_devin_signup_async` (единственный шаг Шага 2 без async-аналога), `process_account_async` (композиция login→baseline→signup→3 попытки кода), `_async_locator_visible`/`_async_read_devin_alert_text`. В `register_devin.py`: `main_async` (оркестрация workers=1 / workers>1 через `asyncio.Semaphore`+`as_completed`; rate-limiting линейной задержкой перед семафором), `_process_account_async_wrapper` (браузер на аккаунт для изоляции cookies). Sync-аналоги (`process_account`, `process_account_worker`, `login_to_mailclient`, `start_devin_signup`, `wait_for_devin_email_code`, `submit_devin_code` + вся sync Playwright-обвязка) остались как DEPRECATED-fallback (помечено в docstring) до живого теста. Тесты: `tests/test_process_account_async.py` (6 сценариев через AsyncMock: happy path, 1 InvalidCodeError→retry, 3 InvalidCodeError→StepError, StepError из login/signup/wait — без ретрая). Итого 139/139 passing. **Ожидается живой тест Шага 2** от пользователя, после чего можно удалить sync-аналоги (этап F, ~700 строк).
...
1. **P2-2 (этап F, блокируется на живом тесте пользователя)** Удалить sync DEPRECATED-fallback: `process_account`, `process_account_worker`, `login_to_mailclient`, `_solve_mailclient_login_captcha`, `start_devin_signup`, `wait_for_devin_email_code`, `submit_devin_code`, `solve_digit_captcha`, sync helper'ы, `from playwright.sync_api`. ~700 строк, после чего `register_devin.py` сократится с ~1790 до ~1100 строк.
