...
## Шаг 4: Проверка карт (`check_cards.py`)

После шагов 1–3 у нас есть зарегистрированные Devin-аккаунты с identity,
но без карт. `check_cards.py` готовит пул «живых» карт для шага 5:
для каждого BIN из `бины.txt` он генерирует тестовые номера, прогоняет
их через [`chkr.cc`](https://chkr.cc) и сохраняет годные в
`живые карты.txt`. Финально перепроверяет их ещё раз и пишет
подтверждённые в `подтверждённые живые карты.txt`.

### Запуск

```cmd
.venv\Scripts\python.exe check_cards.py --quantity 10 --tabs 3
```
...
| `--tabs N` | сколько параллельных вкладок чекинга (default 3). |
| `--check-timeout N` | таймаут одной партии проверки в секундах (default 1200). |
| `--head` / `--headless` | показывать или прятать окно браузера (по умолчанию `--head`). |
| `--no-recheck` | пропустить финальную пере-проверку живых карт. |
| `--keep-existing` | не очищать `живые карты.txt` / `подтверждённые живые карты.txt` перед запуском (по умолчанию они очищаются, чтобы каждый запуск искал заново). |
| `--bins-file PATH` | путь к файлу с BIN-кодами (по умолчанию `бины.txt`). |
| `--browser-mode` | то же что в шагах 1–3 (см. ниже). |

### Что лежит в `бины.txt`

Один BIN в строке — первые 6–8 цифр номера карты. Пустые строки и
строки с `#` игнорируются. Пример:
```
424242
522345
```

`check_cards.py` сам подставит контрольную цифру по алгоритму Луна и
случайные оставшиеся разряды, так что не нужно вводить полный номер.

### Идемпотентность

По умолчанию `живые карты.txt` и `подтверждённые живые карты.txt`
очищаются перед каждым запуском, чтобы пул карт всегда отражал
последнюю проверку. Если ты копишь живые карты между сессиями —
запускай с `--keep-existing`.

## Шаг 5: Активация триалов (`activate_trials.py`)

Последний шаг: входит в каждый Devin-аккаунт, переходит к выбору тарифа,
вводит адрес из `личности.txt`, картой из «живых» проходит экран
Stripe-checkout и решает hCaptcha. Результат успешной активации пишется
в БД как `activated=1`; ошибки — в столбец `last_error`.

В отличие от шагов 1–4 это **async**-pipeline на Camoufox; на каждый
аккаунт поднимается отдельный Firefox-context с собственным временным
профилем под `./temp_profiles/`. После активации профиль удаляется.

### Запуск

```cmd
.venv\Scripts\python.exe activate_trials.py --parallel 3 --timeout 300
```
...
| Флаг | Значение |
|---|---|
| `--parallel N` | одновременных Camoufox-воркеров (default 3). Каждый держит свой Firefox; > 3–4 на 8 GB RAM не рекомендуется. |
| `--timeout SEC` | таймаут активации одного аккаунта (default 300). |
| `--profile PATH` | путь к базовому Firefox-профилю Camoufox; копируется каждому воркеру. |
| `--debug` | детальные debug-логи + скриншоты в `captcha_debug/` при ошибках hCaptcha. |

### Что должно быть готово к шагу 5

- В БД (`accounts.db`) или в `аккаунты devin.txt` — список email+pwd
  зарегистрированных Devin-аккаунтов.
- В `личности.txt` — для каждого email-а его identity (см. шаг 3).
- В `подтверждённые живые карты.txt` — пул живых карт (см. шаг 4).
- `./temp_profiles/` — пустая папка или отсутствует (создастся
  автоматически). На Windows важно, чтобы предыдущий запуск был
  завершён, иначе lock-файлы профилей будут мешать.

### Остановка

GUI'шная «Остановить» ставит `threading.Event`, который проверяется
между шагами активации (вход → checkout → submit). Уже идущий запрос
не отменяется, но новых аккаунтов в работу не берётся. Параллельно
GUI убивает дочерние Firefox-процессы — иначе на Windows lock-файлы
держатся ещё десятки секунд.
